document.addEventListener('DOMContentLoaded', function(){
  ...
  const buildNewFileField = () => {
    ...
    newFileField.setAttribute('name', 'vehicle_fix[images][]');

    // 追加されたfile_fieldにchangeイベントをセット
    newFileField.addEventListener("change", changedFileField);

    // 最後のfile_fieldを取得
    const lastFileField = document.querySelector('input[type="file"][name="vehicle_fix[images][]"]:last-child');
  ...
  // input要素を取得
  const fileField = document.querySelector('input[type="file"][name="vehicle_fix[images][]"]');
